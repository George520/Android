...
def deleteData() #删除数据
    ...
    until $maxId <= location + 1
        @stus[location].setId = "#{@stus[location + 1].getId().to_i - 1}\n"
        @stus[location].setName = @stus[location + 1].getName()
        @stus[location].setGender = @stus[location + 1].getGender()
        @stus[location].setAge = @stus[location + 1].getAge()
        location += 1
    end
    $maxId -= 1
    puts "删除成功"
...
def saveData() #保存数据
    ...
    for i in 0...$maxId #循环结束后i = $ - 1,而不是$maxId
